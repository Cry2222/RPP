...
from config import get_gate_setting, get_random_ua, get_proxy_dict, is_proxy_enabled, blacklist_proxy, get_captcha_setting
...
class _SiteCooldownTracker:
    ...
    def record_block(self, site_url):
        ...
        with self._lock:
            ...
            cooldown = min(120, 30 * block_count)
    ...
    def wait_if_needed(self):
        with self._lock:
            ...
            if now < self._ban_until:
                ...
                time.sleep(wait)
            delay = self._min_delay + (self._backoff_level * random.uniform(1.0, 2.5))
    ...
    def record_rate_limit(self):
        with self._lock:
            ...
            if self._consecutive_errors >= 3:
                self._ban_until = time.time() + 45

    def record_ban(self, duration=90):
        with self._lock:
            self._bans += 1
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if result.get("_cloudflare") and not cloudflare_detected:
                ...
                retry_delay(attempt, base_min=3.0, base_max=6.0)
                continue
            ...
            if result.get("_retry") and attempt < max_retries - 1:
                ...
                retry_delay(attempt, base_min=4.0, base_max=8.0)
                continue
            ...
            if attempt < max_retries - 1:
                retry_delay(attempt, base_min=5.0, base_max=10.0)
        finally:
            try:
                s.close()
            except Exception:
...
def _do_braintree_check(s, cc, mes, ano, cvv, site_url, add_to_cart_path, checkout_path, product_payload, payment_method_id="3", country_code="US", fast_timeout=False):
    ...
    home_timeout = 8 if fast_timeout else 15
    try:
    ...
    if gql_res.status_code == 403:
        snippet = gql_res.text[:500].lower()
        if 'cloudflare' in snippet or 'captcha' in snippet or 'challenge' in snippet:
            captcha_result = auto_solve_page(
                html=gql_res.text,
                page_url=f"{site_url}/braintree/graphql",
                proxy=s.__dict__.get('_proxy_used'),
                session=s,
            )
            if captcha_result.get("solved"):
                logger.info(f"Braintree: CAPTCHA auto-solved via {captcha_result.get('provider','auto')}, retrying")
                return {**_error_result(cc, mes, ano, cvv, "BT CAPTCHA solved - retry needed"), "_retry": True}
...
def setup_braintree_from_url(full_url):
    ...
    try:
        ...
        captcha_info = detect_captcha(home_html, site_url)
