...
STRIPE_WC_SITE_POOL = [
    "https://simplygreatdeals.co.uk",
    "https://on8mil.com",
    "https://rainbows-uniform.co.uk",
    "https://gofrolic.co.uk",
    "https://kebabskee.co.uk",
    "https://nutratea.co.uk",
    "https://ladies-paradise.co.uk",
    "https://supporterstravel.co.uk",
    "https://totemtimber.co.uk",
    "https://palmersquare.com",
    "https://aceclassics.co.uk",
    "https://millfieldshop.com",
    "https://elixirgardensupplies.co.uk",
    "https://revivify.com",
    "https://thetfordgardencentre.co.uk",
    "https://bluerinse.co.uk",
    "https://thecncworkshop.co.uk",
    "https://muschieri.com",
    "https://purpleholly.co.uk",
    "https://hrnews.co.uk",
    "https://tnc-me.com",
    "https://faitholivia.co.uk",
    "https://darwincbd.co.uk",
    "https://theherbalistcompany.co.uk",
    "https://happpyfurniturestudio.co.uk",
    "https://retainerdirect.co.uk",
    "https://diamonddesignbytasha.co.uk",
    "https://littleplumber.com",
    "https://pattersonoil.co.uk",
    "https://compostbaguk.com",
    "https://tokouklospito.gr",
    "https://aspirom.co.uk",
    "https://genxgarage.co.uk",
    "https://allaboutantenatal.co.uk",
    "https://mugandmeeple.co.uk",
    "https://thedollhousecollection.co.uk",
]

BRAINTREE_WC_SITE_POOL = [
    "https://siglent.co.uk",
    "https://simplygreatdeals.co.uk",
    "https://gofrolic.co.uk",
    "https://ladies-paradise.co.uk",
...
def get_next_site(pool_name="stripe"):
    pool = STRIPE_WC_SITE_POOL if pool_name != "braintree" else BRAINTREE_WC_SITE_POOL
    if not pool:
        return None
    return random.choice(pool)


_DEFAULT_STRIPE_AUTH_GATE = {
    "site_url": "https://gofrolic.co.uk",
    "stripe_pub_key": "",
    "account_path": "/my-account/",
}

_DEFAULT_STRIPE_INTENT_GATE = {
    "site_url": "https://supporterstravel.co.uk",
    "stripe_pub_key": "",
    "account_path": "/account/",
}

_DEFAULT_BRAINTREE_AUTH_GATE = {
    "site_url": "https://siglent.co.uk",
    "login_email": "",
    "login_password": "",
    "merchant_id": "wrc3bg2v37npq78r",
    "account_path": "/my-account/",
}

ALL_GATE_TYPES = ("stripe", "braintree", "stripe_auth", "stripe_intent", "braintree_auth")

GATE_TYPE_LABELS = {
    "stripe": "Stripe Charitable",
    "braintree": "Braintree",
...
GATE_TYPE_ALIASES = {
    "st": "stripe", "stripe": "stripe",
    "bt": "braintree", "braintree": "braintree",
    "sa": "stripe_auth", "stripe_auth": "stripe_auth", "stripeauth": "stripe_auth",
    "si": "stripe_intent", "stripe_intent": "stripe_intent", "stripeintent": "stripe_intent",
    "ba": "braintree_auth", "braintree_auth": "braintree_auth", "braintreeauth": "braintree_auth", "btauth": "braintree_auth",
}

def _get_default_for_gate_type(gate_type):
    if gate_type == "braintree":
        return copy.deepcopy(_DEFAULT_BRAINTREE_GATE)
    elif gate_type == "stripe_auth":
        return copy.deepcopy(_DEFAULT_STRIPE_AUTH_GATE)
    elif gate_type == "stripe_intent":
        return copy.deepcopy(_DEFAULT_STRIPE_INTENT_GATE)
    elif gate_type == "braintree_auth":
        return copy.deepcopy(_DEFAULT_BRAINTREE_AUTH_GATE)
    return copy.deepcopy(_DEFAULT_GATE)

GATE_SETTINGS = {
    "stripe": copy.deepcopy(_DEFAULT_GATE),
    "braintree": copy.deepcopy(_DEFAULT_BRAINTREE_GATE),
    "stripe_auth": copy.deepcopy(_DEFAULT_STRIPE_AUTH_GATE),
    "stripe_intent": copy.deepcopy(_DEFAULT_STRIPE_INTENT_GATE),
    "braintree_auth": copy.deepcopy(_DEFAULT_BRAINTREE_AUTH_GATE),
}

_DEFAULT_CAPTCHA_SETTINGS = {
    "enabled": True,
    "auto_solve": True,
    "provider": "capsolver",
...
CAPTCHA_SETTINGS = copy.deepcopy(_DEFAULT_CAPTCHA_SETTINGS)


def get_captcha_setting(key, default=None):
    return CAPTCHA_SETTINGS.get(key, default)


def set_captcha_setting(key, value):
    if key in CAPTCHA_SETTINGS:
        CAPTCHA_SETTINGS[key] = value
        return True
    return False


def get_all_captcha_settings():
    safe = CAPTCHA_SETTINGS.copy()
    if safe.get("api_key"):
        k = safe["api_key"]
        safe["api_key"] = f"{k[:8]}...{k[-4:]}" if len(k) > 12 else "***"
    has_key = bool(CAPTCHA_SETTINGS.get("api_key"))
    safe["mode"] = "API Solver" if has_key else "Free Bypass"
    return safe
...
_gate_configs = {
    ...
    3: {
        ...
        "settings": {
            "site_url": "https://gofrolic.co.uk",
            "stripe_pub_key": "pk_live_51NVYaOCrl9izQZB3dwjOVRfqCjFemkdkkMPnXGgJRJ2VZqvtUPJGa6iQIw03vwgzihXtCJ89ewRi6d9fHTKrzrLS00pLvQbEPJ",
            "account_path": "/my-account/",
        },
        "enabled": True,
        "stats": {"checked": 0, "live": 0, "dead": 0, "errors": 0, "charged": 0},
    ...
    4: {
        "name": "Stripe Intent",
        "gate_type": "stripe_intent",
        "settings": {
            "site_url": "https://supporterstravel.co.uk",
            "stripe_pub_key": "pk_live_51J8O4zH4eiCU8hqLupRZy9HGy59SjbY3kMlwHV8Lwd122n3SOxRC4R7H6U4bD23hCkoEfjkg6fJWsqgGNyx5pMgs00iGwVLRag",
            "account_path": "/account/",
        },
        "enabled": False,
        "stats": {"checked": 0, "live": 0, "dead": 0, "errors": 0, "charged": 0},
    ...
    5: {
        "name": "Braintree Auth",
        "gate_type": "braintree_auth",
        "settings": copy.deepcopy(_DEFAULT_BRAINTREE_AUTH_GATE),
        "enabled": False,
        "stats": {"checked": 0, "live": 0, "dead": 0, "errors": 0, "charged": 0},
    },
}
...
def create_config(name="", from_url="", gate_type="stripe"):
    ...
    if len(_gate_configs) >= MAX_CONFIGS:
        return None, f"Max {MAX_CONFIGS} configs reached"
    if gate_type not in ALL_GATE_TYPES:
        gate_type = GATE_TYPE_ALIASES.get(gate_type.lower(), "stripe")
    cid = _next_config_id
    _next_config_id += 1
    default_settings = _get_default_for_gate_type(gate_type)
    _gate_configs[cid] = {
...
GATE_ENABLED = {"stripe": True, "braintree": True, "stripe_auth": True, "stripe_intent": True, "braintree_auth": True}
...
def set_config_gate_type(config_id, gate_type):
    resolved = GATE_TYPE_ALIASES.get(gate_type.lower(), gate_type.lower()) if gate_type else gate_type
    if config_id in _gate_configs and resolved in ALL_GATE_TYPES:
        old_type = _gate_configs[config_id].get("gate_type", "stripe")
        _gate_configs[config_id]["gate_type"] = resolved
        if old_type != resolved:
            _gate_configs[config_id]["settings"] = _get_default_for_gate_type(resolved)
        return True
    return False
